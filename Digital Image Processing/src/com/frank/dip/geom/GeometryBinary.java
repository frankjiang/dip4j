...
/**
 * The geometry operator for binary image.
 * <p>
 * </p>
 * 
 ...
 * @version 1.0.0
 */
public class GeometryBinary extends Geometry<BinaryImage>
{
	/**
	 * Construct an instance of <tt>GeometryBinary</tt>.
	 */
	public GeometryBinary()
	{
	 ...
	 */
	public GeometryBinary(int type, int fillScheme)
	{
			...
			default:
			{
				for (int x = 0; x < rect.width; x++)
					for (int y = 0; y < rect.height; y++)
					{
						...
						bi.setPixel(x, y, Interpolation.bilinear(ex, ey, p00,
								p01, p10, p11));
					}
			}
				break;
			case TYPE_NEAREST_NEIGHBOR:
			{
				for (int x = 0; x < rect.width; x++)
					for (int y = 0; y < rect.height; y++)
					{
						...
						// -----------------------------------
						bi.setPixel(x, y, image.getPixel(Math.round(p.x),
								Math.round(p.y)));
					}
			}
				break;
			case TYPE_BICUBIC:
			{
				for (int x = 0; x < rect.width; x++)
					for (int y = 0; y < rect.height; y++)
					{
						...
						p10 = image.getPixel(xt + 1, yt);
						p11 = image.getPixel(xt + 1, yt + 1);
						bi.setPixel(x, y, Interpolation.bicubic(ex, ey, p00,
								p01, p10, p11));
					}
			}
				break;
